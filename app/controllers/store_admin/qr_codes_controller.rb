class StoreAdmin::QrCodesController < StoreAdmin::ApplicationController
  def index
  ...
  # Barcode lookup for the booking picker — reports this store's on-hand.
  def lookup
    product = Product.find_by(barcode: params[:barcode].to_s.strip)
    return render(json: { found: false, message: "No product found for barcode: #{params[:barcode]}" }) unless product

    gst_rate = (product.gst_enabled? && product.gst_percentage.present?) ? product.gst_percentage.to_f : 0.0
    response = {
      found: true, id: product.id, name: product.name, sku: product.sku, barcode: product.barcode,
      unit_type: product.unit_type, has_multiple_quantities: product.has_multiple_quantities?,
      gst_enabled: product.gst_enabled?, gst_rate: gst_rate
    }
    ...
    if product.has_multiple_quantities? && product.product_variants.any?
      ...
      response[:available_stock] = @current_store.available_stock_for(product.id).to_i
    end
